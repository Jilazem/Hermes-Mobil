...
class Handler(BaseHTTPRequestHandler):
    ...
    def _token_ok(self) -> bool:
        ...
        if ARGS.token_any:
            # Tur-12: emülatördeki uygulamanın profil tokeni gerçek olabilir;
            # kanıt koşusunda değeri OKUMAK/SAKLAMAK yerine yalnız varlığını ve
            # geri döndürülemez bir özetini kaydediyoruz (token sızmaz).
            digest = hashlib.sha256(got.encode("utf-8")).hexdigest()[:8] if got else "-"
            log(f"{self.command} {self.path} · token_any · sha8={digest} len={len(got)}")
            return True
    ...
    def _synthesize(self, body: bytes) -> None:
        ...
        log(
            f"200 POST /synthesize · motor={engine} metin=\"{text}\" uzunluk={len(text)} "
            f"oggg_bayt={len(OGG)}"
        )
...
def main() -> int:
    ...
    ap.add_argument(
        "--token-any",
        action="store_true",
        help="Tur-12: token kapısını aç (uygulamanın tokeni bilinmiyorsa); "
             "yalnız varlık + sha8 kaydedilir, değer saklanmaz.",
    )
